...
void gpgpusim_cuda_launchDeviceV2(const ptx_instruction * pI, ptx_thread_info * thread, const function_info * target_func) {
    ...
    for( unsigned arg=0; arg < n_args; arg ++ ) {
	    ...
	    if (device_kernel_entry->is_pdom_set()) {
            ...
            gpgpu_t* gpu=thread->get_gpu();   //SUCHITA
//            device_grid = new kernel_info_t(config.grid_dim, config.block_dim, device_kernel_entry, gpu->getNameRefMapping(),gpu->getRefNameMapping(),gpu->getNameArrayMapping(),gpu->getNameInfoMapping(),gpu->getNameAttrMapping());   //SUCHITA
	    device_grid = new kernel_info_t(config.grid_dim, config.block_dim, device_kernel_entry, gpu->getNameArrayMapping(), gpu->getNameInfoMapping());   //SUCHITA
